# -*- coding: utf-8 -*-
"""
Created on Mon Aug 24 20:33:35 2015
...
class NeuralAgent(object):
    ...
    def start_episode(self, observation, testing=False):
        """
        ...
        return_action = 0#self.randGenerator.randint(0, self.num_actions-1)

#TODO: cia neaisku kas gaunasi jei iskart gameover ar pan. Dabar ne gameover action 0
        #return_action = self._choose_action(self.test_data_set, .001,
        #                                 observation, np.clip(0, -1, 1), True)
        if testing:
            return_action = self._choose_action(self.test_data_set, .001,
                                         observation, np.clip(0, -1, 1), True)
    ...
    def step(self, reward, observation):
        ...
        if self.testing:
            ...
            action = self._choose_action(self.test_data_set, .001,
                                         observation, np.clip(reward, -1, 1), True)
#            #TEST                             
#            if observation[0]<0 and observation[2]>0:
#                action=2
#            if observation[0]>0 and observation[2]>0:
#                action=1
#            if observation[2]<0:
#                action=3       
    ...
    def _choose_action(self, data_set, epsilon, cur_img, reward, training=False):
        """
        ...
        if self.step_counter >= 0:#self.phi_length: siaip buvo>phi
#ima daugiau frames is istorijos, mano atveju geriau 10s, 1min, 5min
...
#        net_file.close()
        ...
        print("total train reward: " + str(self.total_reward))
        if self.total_reward>7.5:
            1==1
        print("epsilon: "+str(self.epsilon) )    
        print("mean loss"+ str(np.mean(self.loss_averages)))
        print("")
    ...
    def finish_testing(self, epoch):
        ...
        print("total test reward: " + str(self.total_reward))
        print("")
                                  
        #if self.total_reward>5:
            #kazka daryt, reiktu je dar moko, tai mokyme daug geru pvz
